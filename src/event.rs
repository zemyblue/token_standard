...
impl<'a> Event for TransferEvent<'a> {
    fn add_attribute(&self, rsp: &mut Response) {
        ...
        rsp.attributes
            .push(attr("recipient", self.recipient.to_string()));
...
impl<'a> Event for ApprovalEvent<'a> {
    fn add_attribute(&self, rsp: &mut Response) {
        ...
        rsp.attributes.push(attr("owner", self.owner.to_string()));
        rsp.attributes
            .push(attr("spender", self.spender.to_string()));
        rsp.attributes
            .push(attr("old_amount", self.old_amount.to_string()));
        rsp.attributes
            .push(attr("new_amount", self.new_amount.to_string()));
    }
}
